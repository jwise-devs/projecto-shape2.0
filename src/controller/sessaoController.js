...
exports.data = async (req, res) => {
  try {
    ...
    const precosTratamentos = JSON.parse(req.body.preco_tratamentos).map((p) =>
      parseFloat(p),
    );
    const tratamentosJSON = Array.isArray(tratamentos)
      ? tratamentos
      : [tratamentos];
    ...
    const sessoesAnteriores = await Sessao.findAll({
      where: { userId: usuarioId },
    });
    const totalTratamentosAnteriores = sessoesAnteriores.reduce(
      (acc, sessao) => acc + (sessao.tratamentosArray?.length || 0),
      0,
    );
    const totalSessao =
      Number(totalTratamentosAnteriores) + tratamentosJSON.length;
    ...
    req.session.save(() => res.redirect(`/sessao/${sessao.id}/pagar`));
  } catch (error) {
    console.error(error);
    if (error.errors) {
...
exports.novaSessao = async (req, res) => {
  try {
    ...
    tratamentosData.forEach((t) => {
      ...
      const existente = tratamentosFiltrados.find(
        (trat) => trat.nome === nome && Number(trat.preco) === preco,
      );
...
exports.criarSessaoParaUsuario = async (req, res) => {
  try {
    ...
    if (!resultado || resultado.trim() === "") {
      resultado =
        pacotePersonalizado && pacotePersonalizado.trim() !== ""
          ? pacotePersonalizado
          : "Personalizado";
    }
